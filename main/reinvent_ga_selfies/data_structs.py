...
class Vocabulary(object):
    ...
    def __init__(self, init_from_file=None, max_length=140):
        ...
        if init_from_file: self.init_from_file(init_from_file) ### data/Voc 
    ...
    def tokenize(self, selfies):
        ...
        char_list = ['['+word+']' for word in words]
        tokenized = []
...
class Experience(object):
    ...
    def rank_based_sample(self, n, rank_coefficient=0.01):
        ...
        if len(self.memory)<n:
            ...
            scores_np = np.array([x[1]+1e-10 for x in self.memory])
        ...
        encoded = MolData.collate_fn(encoded)
        return encoded, np.array(valid_scores)
